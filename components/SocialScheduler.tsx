...
const SocialScheduler: React.FC<SocialSchedulerProps> = ({ products, scheduledPosts, setScheduledPosts, preselectedProduct, onClearPreselected }) => {
        ...
        <div className="flex gap-2 bg-ios-card p-1.5 rounded-2xl border border-ios shadow-sm self-start md:self-auto w-full md:w-auto overflow-x-auto no-scrollbar">
             {[
                 ...
                 <button 
                    ...
                    className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap spring-press ${
                        activeTab === tab.id ? 'bg-ios-accent text-white shadow-lg' : 'text-ios-secondary hover:bg-ios-sub'
                    }`}
                 >
            ...
            <div className="px-1 pb-2">
              <div className="flex justify-between items-start mb-2">
                 <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md border ${
                     post.status === 'draft' ? 'bg-amber-100 text-amber-600 border-amber-200' :
