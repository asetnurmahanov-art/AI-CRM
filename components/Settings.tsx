...
const Settings: React.FC<SettingsProps> = ({ companies, activeCompany, updateCompany, currentTheme, setTheme }) => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
           {themes.map((t) => (
             <button 
               ...
               className={`relative flex flex-col items-center p-4 rounded-3xl transition-all spring-press border-2 group overflow-hidden ${
                 currentTheme === t.id ? 'border-ios-accent shadow-xl scale-105' : 'border-transparent hover:bg-ios-sub'
               }`}
             >
               {/* Color Preview Blob */}
